...
MODELOS_GEMINI = ['gemini-1.5-flash', 'gemini-1.5-pro-preview']
    ...
    try:
        ...
        try:
            ...
            for w in words_detail_json:
                ...
                # Extracción mejorada de prosodia según documentación de Azure
...
def nuevo_texto_lectura():
    try:
        user_id, err = obtener_usuario_autenticado_y_suscrito()
        if err: return jsonify({'error': err}), 401
        prompt = """Genera una lectura en inglés de nivel intermedio (30 palabras aprox). 
        Devuelve ÚNICAMENTE un objeto JSON válido con este formato exacto, sin marcas de código Markdown:
        {"titulo": "Titulo", "texto": "Texto", "traduccion": "Traduccion", "preguntas": [{"pregunta": "Pregunta", "opciones": ["A", "B", "C", "D"], "correcta": 1, "explicacion": "Breve"}]}"""
...
def evaluar_dictado():
    try:
        ...
        res_text = generar_contenido_gemini(prompt)
        if res_text: return jsonify({'calificacion': 6, 'analisis': res_text})
